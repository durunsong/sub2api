...
const mountModal = (realSelect = false) => mount(BulkEditKeysModal, {
  props: {
    show: true,
...
describe('BulkEditKeysModal', () => {
  ...
  it('finds Kiro groups by Claude and submits the unchanged group ID', async () => {
    const wrapper = mountModal(true)
    await wrapper.setProps({ groups: [
      { id: 7, name: '高级分组', platform: 'kiro' },
      { id: 8, name: 'OpenAI group', platform: 'openai' },
    ] as Group[] })
    await wrapper.get('[data-test="enable-group"]').setValue(true)
    await wrapper.get('[data-test="group-input"] button').trigger('click')
    const input = document.body.querySelector<HTMLInputElement>('.select-search-input')!
    input.value = 'claude'
    input.dispatchEvent(new Event('input', { bubbles: true }))
    await flushPromises()
    const options = document.body.querySelectorAll<HTMLElement>('[role="option"]')
    expect([...options].map(option => option.textContent?.trim())).toEqual(['高级分组'])
    options[0].click()
    await flushPromises()
    await wrapper.get('form').trigger('submit')
    await flushPromises()
