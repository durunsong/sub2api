  ...
  return mount(AvailableChannelsTable, {
    ...
    global: {
      ...
      stubs: {
        ...
        GroupBadge: {
          props: ['name', 'showRate', 'userFacing'],
          template: '<span data-group-badge :data-show-rate="String(showRate)">{{ name }}</span>',
        },
        SupportedModelChip: {
...
describe('AvailableChannelsTable responsive surfaces', () => {
  ...
  it('renders a mobile-only readable surface without exposing group multipliers', () => {
    const wrapper = mountTable()
    ...
    expect(mobile.findAll('[data-group-badge]').every((badge) => badge.attributes('data-show-rate') === 'false')).toBe(true)
