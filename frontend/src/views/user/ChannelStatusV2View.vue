...
async function loadTab(signal?: AbortSignal, id = sequence) {
  tabController?.abort()
  const request = signal ? null : new AbortController()
  tabController = request
  const tabSignal = signal || request?.signal
  const tab = activeTab.value
  const tabId = ++tabSequence
  tabLoading.value = true
  try {
    let items: MonitorModelRow[] | MonitorErrorRow[] | MonitorUserRow[]
    if (tab === 'models') {
      items = (await api.getModels(filter.value, isAdmin.value, tabSignal)).items || []
    } else if (tab === 'errors') {
      items = (await api.getErrors(filter.value, isAdmin.value, tabSignal)).items || []
    } else {
      items = (await api.getUsers(filter.value, isAdmin.value, tabSignal)).items || []
    }
    if (id !== sequence || tabId !== tabSequence || tab !== activeTab.value) return
    if (tab === 'models') modelRows.value = items as MonitorModelRow[]
    else if (tab === 'errors') errorRows.value = items as MonitorErrorRow[]
    else userRows.value = items as MonitorUserRow[]
  } catch (error) {
    const e = error as { name?: string; code?: string }
    if (e?.name === 'AbortError' || e?.name === 'CanceledError' || e?.code === 'ERR_CANCELED') return
    if (id === sequence && tabId === tabSequence && tab === activeTab.value) {
      appStore.showError(extractApiErrorMessage(error, t('channelMonitorV2.detailLoadFailed')))
    }
  } finally {
    if (id === sequence && tabId === tabSequence && tab === activeTab.value) tabLoading.value = false
  }
}
