...
describe('admin UsersView', () => {
  ...
  it('cancels bulk deletion without deleting or clearing selected users', async () => {
    ...
    expect(wrapper.get('[data-test="delete-dialog"]').text()).toContain('admin.users.batchDeleteConfirm:1')
  ...
  it.each([
    ...
    batchDeleteUsers.mockImplementation(async (ids: number[]) => ({
      deleted_ids: ids.filter(id => !failedIds.includes(id)),
      skipped: failedIds.map(id => ({ id, reason: 'admin_user' }))
    }))
    const wrapper = mountBulkDeleteView()
    await flushPromises()
    await wrapper.get('[data-test="select-42"]').trigger('click')
    ...
    expect(wrapper.find('[data-test="delete-dialog"]').exists()).toBe(false)
    if (deleted) {
      expect(showSuccess).toHaveBeenCalledWith('admin.users.batchDeleteDone')
      expect(listUsers.mock.lastCall?.[0]).toBe(1)
    } else {
      expect(showSuccess).not.toHaveBeenCalled()
    }
    if (!deleted && failedIds.length) expect(showInfo).toHaveBeenCalledWith('admin.users.batchDeleteSkipped')
  ...
  it('deletes the confirmed selection while preserving users selected during deletion', async () => {
    ...
    batchDeleteUsers.mockImplementation(() => new Promise(resolve => {
      finishDelete = () => resolve({ deleted_ids: [42], skipped: [] })
    }))
    const wrapper = mountBulkDeleteView()
    await flushPromises()
    await wrapper.get('[data-test="select-42"]').trigger('click')
