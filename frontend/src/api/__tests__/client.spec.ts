...
describe('API Client', () => {
  ...
  describe('401 Token 刷新', () => {
    ...
    it.each([403, 429, 500, 503, 0])('刷新暂时失败（%s）时保留会话并返回实际状态', async (status) => {
      localStorage.setItem('auth_token', 'expired-token')
      localStorage.setItem('refresh_token', 'refresh-token')
      localStorage.setItem('auth_user', JSON.stringify({ id: 7 }))
    ...
    it('刷新返回非 HTTP 异常时保留当前会话', async () => {
      localStorage.setItem('auth_token', 'expired-token')
      localStorage.setItem('refresh_token', 'refresh-token')
      localStorage.setItem('auth_user', JSON.stringify({ id: 7 }))
      const refresh = await import('@/api/tokenRefresh')
      vi.spyOn(refresh, 'refreshAuthTokens').mockRejectedValueOnce(new Error('Invalid refresh response'))
      apiClient.defaults.adapter = vi.fn().mockRejectedValueOnce({
        response: { status: 401, data: { code: 'TOKEN_EXPIRED' } },
        config: { url: '/test', headers: { Authorization: 'Bearer expired-token' } },
      })
      await expect(apiClient.get('/test')).rejects.toMatchObject({ status: 0, code: 'TOKEN_REFRESH_UNAVAILABLE' })
      expect(localStorage.getItem('auth_token')).toBe('expired-token')
      expect(localStorage.getItem('refresh_token')).toBe('refresh-token')
      expect(localStorage.getItem('auth_user')).toBe(JSON.stringify({ id: 7 }))
      expect(sessionStorage.getItem('auth_expired')).toBeNull()
    ...
    it.each([401])('刷新被拒绝（%s）时仍清除失效会话', async (status) => {
    ...
    it('刷新服务临时故障时保留当前会话', async () => {
      ...
      await expect(apiClient.get('/test')).rejects.toMatchObject({ status: 503 })
      expect(localStorage.getItem('auth_token')).toBe('expired-token')
      expect(localStorage.getItem('refresh_token')).toBe('refresh-token')
      expect(localStorage.getItem('auth_user')).toBe(JSON.stringify({ id: 7 }))
    })
