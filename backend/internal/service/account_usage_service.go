...
func ProvideAccountUsageService(
	...
	return NewAccountUsageService(
		...
		identityCache,
		tlsFPProfileService,
	).SetOpenAIQuotaService(openAIQuotaService).SetGrokQuotaService(grokQuotaService).SetKiroTokenProvider(kiroTokenProvider)
}

func (s *AccountUsageService) SetOpenAIQuotaService(provider *OpenAIQuotaService) *AccountUsageService {
	if s != nil {
		s.openAIQuotaService = provider
	}
	return s
}

func (s *AccountUsageService) SetGrokQuotaService(provider *GrokQuotaService) *AccountUsageService {
	if s != nil {
		s.grokQuotaService = provider
	}
	return s
}
