...
func ProvideTokenRefreshService(
	...
	svc := NewTokenRefreshService(accountRepo, oauthService, openaiOAuthService, geminiOAuthService, antigravityOAuthService, grokOAuthService, kiroOAuthService, cacheInvalidator, schedulerCache, cfg, tempUnschedCache)
...
func ProvideAccountUsageService(
	...
	service.SetGrokQuotaService(grokQuotaService).SetKiroTokenProvider(kiroTokenProvider)
	service.agentIdentityWS = openAIGatewayService
	return service
}
...
// ProvidePluginManager keeps plugin host services wired across Wire regeneration.
...
func ProvidePaymentService(entClient *dbent.Client, registry *payment.Registry, loadBalancer payment.LoadBalancer, redeemService *RedeemService, subscriptionSvc *SubscriptionService, configService *PaymentConfigService, userRepo UserRepository, groupRepo GroupRepository, affiliateService *AffiliateService, notificationEmailService *NotificationEmailService, balanceCacheInvalidator *BillingCacheService) *PaymentService {
	svc := NewPaymentService(entClient, registry, loadBalancer, redeemService, subscriptionSvc, configService, userRepo, groupRepo, affiliateService, balanceCacheInvalidator)
	svc.SetNotificationEmailService(notificationEmailService)
	return svc
}
