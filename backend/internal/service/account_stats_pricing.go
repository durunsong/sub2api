...
// pricingAt 是请求级冻结计价时刻，用于优先级 3 的模型分时策略；同时与客户计费保持一致，避免跨峰谷错位。
...
func resolveAccountStatsCost(
	...
	if billingService != nil {
		return tryModelFilePricing(ctx, billingService, upstreamModel, tokens, serviceTier, pricingAt, reasoningEffort)
	}
...
// 每加一个定价特性都要手工镜像一次。Resolver 不注入 ChannelService，保持
// 优先级 3 的语义：只取模型定价文件，不引入分组或渠道自定义定价。
