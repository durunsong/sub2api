...
func TestTryModelFilePricing_Success(t *testing.T) {
	...
	tokens := UsageTokens{InputTokens: 100, OutputTokens: 50}
	result := tryModelFilePricing(context.Background(), bs, "claude-sonnet-4", tokens, "", time.Time{})
	require.NotNil(t, result)
	// 100*0.001 + 50*0.002 = 0.1 + 0.1 = 0.2
	require.InDelta(t, 0.2, *result, 1e-12)
}
...
func TestTryModelFilePricing_Fable51MaxEffortUsesTripleQuota(t *testing.T) {
	...
	tokens := UsageTokens{InputTokens: 100}
	standard := tryModelFilePricing(context.Background(), bs, "claude-fable-5-1", tokens, "", time.Time{}, "xhigh")
	max := tryModelFilePricing(context.Background(), bs, "claude-fable-5-1", tokens, "", time.Time{}, "max")
...
func TestTryModelFilePricing_AppliesLongContextPricing(t *testing.T) {
	...
	result := tryModelFilePricing(context.Background(), bs, "gpt-5.6-sol", tokens, "", time.Time{})
...
func TestTryModelFilePricing_AppliesDeepSeekPeakPricingAt(t *testing.T) {
	bs := newTestBillingServiceWithPrices(map[string]*ModelPricing{
		"deepseek-v4-flash": {
			InputPricePerToken:     2.2e-7,
			OutputPricePerToken:    6.6e-7,
			CacheReadPricePerToken: 7e-9,
		},
	})
	tokens := UsageTokens{InputTokens: 1_000, OutputTokens: 500, CacheReadTokens: 1_000}
...
func TestTryModelFilePricing_AppliesServiceTierPricing(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := tryModelFilePricing(context.Background(), bs, "gpt-5.6-sol", tokens, tt.serviceTier, time.Time{})
			require.NotNil(t, result)
...
func TestTryModelFilePricing_CombinesPriorityAndLongContextPricing(t *testing.T) {
	...
	result := tryModelFilePricing(context.Background(), bs, "gpt-5.6-sol", tokens, "priority", time.Time{})
...
func TestTryModelFilePricing_PricingNotFound(t *testing.T) {
	...
	bs := newTestBillingServiceWithPrices(map[string]*ModelPricing{})
	tokens := UsageTokens{InputTokens: 100, OutputTokens: 50}
	result := tryModelFilePricing(context.Background(), bs, "nonexistent-model", tokens, "", time.Time{})
	require.Nil(t, result)
}
