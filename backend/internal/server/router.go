...
func SetupRouter(
	...
	if web.HasEmbeddedFrontend() {
		web.RegisterEmbeddedFrontend(r, settingService, settingService.SetOnUpdateCallback, refreshFrameOrigins)
	} else {
		settingService.SetOnUpdateCallback(refreshFrameOrigins)
	}

	// 注册路由
	registerRoutes(r, handlers, jwtAuth, optionalJWTAuth, adminAuth, apiKeyAuth, auditLog, stepUpAuth, apiKeyService, subscriptionService, opsService, settingService, ipBanService, compositeResolver, cfg, redisClient)
...
func registerRoutes(
	...
	// 注册各模块路由
	routes.RegisterAuthRoutes(v1, h, jwtAuth, auditLog, redisClient, settingService, ipBanService, cfg, panelRateLimiter)
	...
	routes.RegisterGatewayRoutes(r, h, apiKeyAuth, apiKeyService, subscriptionService, opsService, settingService, compositeResolver, cfg, ipBanService)
